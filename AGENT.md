...
- `multi_clip_sequence=true` only with strong evidence that unrelated source clips are sequentially stitched together (different cats/locations/camera sources etc.); ordinary movement/reframing/time progression is not enough.
...
- Reject memory is version-aware: old obvious branding/UI/caption rejects remain durable; stale v1 collage/compilation-only rejects may be rechecked once under v2. Deterministic preview format rejects (`youtube_preflight_rejects`, `durable_reject=true`) are also durable.
- Transient preview decode/tool failures must not become permanent reject-memory entries.
...
Slot 2 cat checkpoints:

1. Impact cards / real meow / no voice / no BGM are accepted.
2. Narrow themes were rejected in favor of generic `#001 — Котики`.
3. Near-9:16 stock baseline is locally confirmed with six 720×1280 Pexels clips; generic vertical render was accepted as normal.
4. Official YouTube Data API CC discovery works locally.
5. Three Pawcsu clips were correctly rejected for source branding + large captions.
6. `nWieRK7Fw-g` was correctly rejected for livestream/chat UI + branding/captions.
7. `hxXfevBB9Zs` was rechecked with contact-sheet-aware v2 and correctly rejected as a real stitched multi-clip sequence.
8. Current saved report has two new ranks: `cQE-s_wsclw` (#1) and `I_pdwiLlvuc` (#2).
9. Candidate #1 `cQE-s_wsclw` passed clean preflight but full validation found **502×720**, outside near-9:16 tolerance. This exposed a late ordering bug, not a reason to relax the vertical rule.
10. Deterministic low-res preview geometry/duration preflight is now implemented before Luna/full download, with durable reject-memory for format failures. Immediate local action: pull/reinstall/test, re-run current candidate #1 once to register the aspect reject, then test current candidate #2 without rerunning search.
11. Keep Pexels/Pixabay as licensed fallback. Only after cat sourcing is accepted, return to slot 3 / remaining pilot.
