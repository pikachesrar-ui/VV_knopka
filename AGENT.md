...
- Official `cc-import` must recheck `status.license == creativeCommon`, then use a low-resolution temporal preview before downloading production-quality media. Full-quality download happens only after low-res clean preflight PASS.
...
- Reject memory is version-aware: old obvious branding/UI/caption rejects remain durable; stale v1 collage/compilation-only rejects may be rechecked once under v2.
...
3. Near-9:16 gate is locally confirmed with six 720×1280 stock clips.
4. User's verdict on the generic vertical render: **«да, норм»**.
5. Official YouTube Data API CC discovery works locally.
6. Three Pawcsu imports were API-confirmed CC/vertical/audible but correctly rejected by clean gate for visible branding + large captions.
7. `nWieRK7Fw-g` passed thumbnail prescreen but was correctly rejected later for livestream/chat UI, branding and Korean captions; this motivated low-res temporal preflight.
8. Current saved report candidate `hxXfevBB9Zs` passed low-res preflight at 0.98, then old v1 full gate rejected it because the generated 2x2 contact sheet was interpreted as a 2x2 compilation. Treat that verdict as potentially false-positive.
9. Clean-footage v2 + CLI v6 fix this ambiguity. Immediate next local action: pull/reinstall/test, then re-run `cc-import 2 --candidate 1` from the current saved report without rerunning search first.
10. Keep Pexels/Pixabay as licensed fallback. Only after cat sourcing is accepted, return to slot 3 / remaining pilot.
