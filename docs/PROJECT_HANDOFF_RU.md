...
`longrun-task.log`. Не запускать ещё три генерации и не ослаблять gates до
просмотра этого журнала. 403 от 03:32 — отдельный ночной запуск; нельзя
...
создал ZIP `vv-analytics-20260920-173107.zip` и позже прикрепил его; без
журнала runner не выводить точную причину остановки
сегодняшнего ярлыка. Более ранний ночной запуск 03:32 остановился на Responses
...
уже задаёт UTF-8, поэтому его вывод эмодзи остаётся прежним. Для определения,
почему вручную сегодня вышло меньше запрошенных видео, нужны свежие
`manual-batch-state.json`, хвост `manual-batch.log` и строки `longrun-task.log`
за последний запуск. Повторные нажатия без диагноза могут расходовать бюджет.
